...
<div class="tbl_head01 tbl_wrap">
    <table>
        ...
        for ($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            // [오직 이 부분만 수정됨] 유형별 대수를 표시합니다. 
            $types = array();
            if($row['pi_type_general']) {
                $types[] = '일반: ' . number_format((int)$row['pi_capa_general']) . '대';
            }
            if($row['pi_type_barrier']) {
                $b_capa = isset($row['pi_capa_barrier']) ? $row['pi_capa_barrier'] : (isset($row['pi_capa_pregnant']) ? $row['pi_capa_pregnant'] : 0);
                $types[] = '베리어프리: ' . number_format((int)$b_capa) . '대';
            }
            if($row['pi_type_large']) {
                $types[] = '대형: ' . number_format((int)$row['pi_capa_large']) . '대';
            }
        ...
        <tr class="<?php echo $bg; ?>">
            ...
            <td class="td_left" style="font-size:0.95em; line-height:1.5;"><?php echo implode('<br>', $types); ?></td>
